...
def embed_themes(
    ...
    already_stored = {
        row["theme"]
        for row in (
            supabase.table(embeddings_table).select("theme").execute().data or []
        )
        if row.get("theme")
    }
